...
class DiagnosticsRecorder(root: File) {
    private val sessionDir = File(root, "session-" + SimpleDateFormat("yyyyMMdd-HHmmss", Locale.US).format(Date())).apply { mkdirs() }
    ...
    fun record(frame: OcrFrame, events: List<GameEvent>, notes: List<String>, bitmap: Bitmap) {
        ...
        if (images < MAX_IMAGES && (periodic || eventImage)) {
            ...
            FileOutputStream(File(sessionDir, "frame-%04d.jpg".format(frames))).use { scaled.compress(Bitmap.CompressFormat.JPEG, 60, it) }
    ...
    companion object {
        ...
        fun share(context: Context): Boolean {
            ...
            val intent = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_SUBJECT, text.getString(R.string.diagnostics_subject))
